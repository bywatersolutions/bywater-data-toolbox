...
#   $debug and print Dumper(@data);
   ...
   ## Adds ignore to UPDATE statement
      if ($ignoreflag) {
	 $querystr = "UPDATE IGNORE $table_name SET ";
      }
   my $exception = 0;
   for (my $i=1;$i<scalar(@data);$i++){
      ...
      if ($fields[$i] eq $bibliocol){
         my $convertq = $dbh->prepare("SELECT biblionumber FROM biblio WHERE $bibliocol = '$data[$i]';");
         $convertq->execute();
         my $rec=$convertq->fetchrow_hashref();
         if ($rec->{'biblionumber'}){
